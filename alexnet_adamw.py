...
train_dataset = datasets.ImageFolder(root='./trainDataset/', transform=transformation)

# Dictionary to map images to classes
class_map = {row["image_title"]: row["class_label"] for index, row in train_dataframe.iterrows()}
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_title = os.path.basename(self.dataset.dataset.imgs[original_index][0])
...
model.classifier[6] = nn.Linear(num_ftrs, 4)  # Adjust the number of classes
...
for epoch in range(100):
    ...
    # Add calculation of validation loss and validation acccuracy
    test_accuracy, test_loss = evaluate(model, test_dataloader, device, criterion)
    ...
    if epochs_that_min_val_loss_does_not_reduce == 15: 
        print("Stopping early as validation loss didn't reduce for 15 consecutive epochs.")
        break
